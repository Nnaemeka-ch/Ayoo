...
from constants import CANVAS_WIDTH, CANVAS_HEIGHT, BOARD_COLOR, INNER_COLOR, POT_COLOR, SEED_COLOR
...
class Okwe:
    def __init__(self, root, canvas_width=CANVAS_WIDTH, canvas_height=CANVAS_HEIGHT):
        self.root = root
    ...
    def game_area_sqr(self):
        ...
        self.canvas.create_rectangle(self.x1, self.y1, self.x2, self.y2, fill=BOARD_COLOR, outline="")
        self.canvas.create_rectangle(self.x3, self.y3, self.x4, self.y4, fill=INNER_COLOR, outline="")
    ...
    def draw_game_pot_at(self):
        ...
        def draw_pot_and_seeds(cx, cy, pot_number):
            # Draw the pot
            self.canvas.create_oval(cx - self.radius, cy - self.radius, cx + self.radius, cy + self.radius, fill=POT_COLOR, outline="")
